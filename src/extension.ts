...
async function runPythonBackend(code: string): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
        ...
        proc.on('close', code => {
            if (code !== 0) reject(new Error(stderr || 'Backend crashed'));
            else {
                try { resolve(JSON.parse(stdout)); } 
                catch (e) { reject(new Error('Backend returned invalid JSON')); }
            }
        });
    });
}
